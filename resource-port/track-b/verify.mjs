...
for (let index = 0; index < map.sessions.length; index += 1) {
  ...
  if (seenOwners.has(session.owner)) fail(`duplicate owner ${session.owner}`);
  seenOwners.add(session.owner);
  ...
  if (!prompts.includes(`## ${id} —`)) fail(`${id} prompt missing`);
  if (!prompts.includes(`RESOURCE_SESSION_ID=${id}`)) fail(`${id} exact prompt id missing`);
  for (const family of session.sourceFamilies) {
    if (typeof family !== "string" || family.length === 0) fail(`${id} invalid source family`);
  }
}

if (seenOwners.size !== 11) fail("owner count mismatch");
if (edges !== 36) fail(`expected 36 dependency edges, found ${edges}`);

console.log(JSON.stringify({
  status: "pass",
  sessions: map.sessions.length,
  owners: seenOwners.size,
