...
1. Read `RESOURCE_PORT_TRACK_B.md`, `resource-port/track-b/session-map.json` and the exact `RNNN` section in `resource-port/track-b/SESSION_PROMPTS.md` before changing files.
...
3. Treat source asset roots as read-only:
   ...
   - `weekly/`
   - `xml/`
4. Write generated or curated outputs only to the exact session entry's `lockedPaths` in `resource-port/track-b/session-map.json`.
...
10. Use exactly two logical commits:
    - `RNNN` for package, contract, evidence, checker and the exact session-map entry update.
